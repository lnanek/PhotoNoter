...
public class SPen_Example_ImageFilter extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mBackgroundImageView.setImageBitmap(bmBackgroundBitmap);
			...
			public void onItemClick(AdapterView parent, View view, int position, long id) {
				...
				{						
					...
					if(bmFilteredBitmap!=null){    			
						mBackgroundImageView.setImageBitmap(bmFilteredBitmap);    				
					}			
					else
					{
	...
	protected void onDestroy() {
		...
		if(bmBackgroundBitmap != null){
			bmBackgroundBitmap.recycle();
			bmBackgroundBitmap = null;
		}

		if(bmFilteredBitmap != null){
			bmFilteredBitmap.recycle();
			bmFilteredBitmap = null;
		}		
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(resultCode==RESULT_OK) {
			...
			if(requestCode == REQUEST_CODE_SELECT_IMAGE_BACKGROUND) {    			
				...
				bmBackgroundBitmap = SPenSDKUtils.getSafeResizingBitmap(strBackgroundImagePath, mBackgroundImageView.getWidth()/2, mBackgroundImageView.getHeight()/2, true);
				if(bmBackgroundBitmap!=null){    			
					mBackgroundImageView.setImageBitmap(bmBackgroundBitmap);    				
				}
				else
