	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mSCanvas.setSettingStrokeChangeListener( new SettingStrokeChangeListener() {
			...
			public void onBeautifyPenStyleParameterCursiveChanged(int cursiveParameter) {	
				if(mCurrentTool == TOOL_PEN)
					mStrokeInfoPen.setBeautifyStyleParamValue(SObjectStroke.BEAUTIFY_STYLE_PARAM_INDEX_CS, cursiveParameter);
				else if(mCurrentTool == TOOL_FINGER)
					mStrokeInfoFinger.setBeautifyStyleParamValue(SObjectStroke.BEAUTIFY_STYLE_PARAM_INDEX_CS, cursiveParameter);
			}

			@Override
			public void onBeautifyPenStyleParameterDummyChanged(int dummyParamter) {
				if(mCurrentTool == TOOL_PEN)
					mStrokeInfoPen.setBeautifyStyleParamValue(SObjectStroke.BEAUTIFY_STYLE_PARAM_INDEX_QB, dummyParamter);
				else if(mCurrentTool == TOOL_FINGER)
					mStrokeInfoFinger.setBeautifyStyleParamValue(SObjectStroke.BEAUTIFY_STYLE_PARAM_INDEX_QB, dummyParamter);
			}

			@Override
			public void onBeautifyPenStyleParameterModulationChanged(int modulationParamter) {	
				if(mCurrentTool == TOOL_PEN)
					mStrokeInfoPen.setBeautifyStyleParamValue(SObjectStroke.BEAUTIFY_STYLE_PARAM_INDEX_YY, modulationParamter);
				else if(mCurrentTool == TOOL_FINGER)
					mStrokeInfoFinger.setBeautifyStyleParamValue(SObjectStroke.BEAUTIFY_STYLE_PARAM_INDEX_YY, modulationParamter);
			}

			@Override
			public void onBeautifyPenStyleParameterSustenanceChanged(int sustenanceParamter) {	
				if(mCurrentTool == TOOL_PEN)
					mStrokeInfoPen.setBeautifyStyleParamValue(SObjectStroke.BEAUTIFY_STYLE_PARAM_INDEX_BM, sustenanceParamter);
				else if(mCurrentTool == TOOL_FINGER)
					mStrokeInfoFinger.setBeautifyStyleParamValue(SObjectStroke.BEAUTIFY_STYLE_PARAM_INDEX_BM, sustenanceParamter);
			}

			@Override
			public void onBeautifyPenStyleParameterBeautifyStyleIDChanged(int styleID) {	
				if(mCurrentTool == TOOL_PEN)
					mStrokeInfoPen.setBeautifyID(styleID);
				else if(mCurrentTool == TOOL_FINGER)
					mStrokeInfoFinger.setBeautifyID(styleID);
			}
			@Override
			public void onBeautifyPenStyleParameterFillStyleChanged(int fillStyle) {				
				if(mCurrentTool == TOOL_PEN)
					mStrokeInfoPen.setBeautifyLineFillStyleIndex(fillStyle);
				else if(mCurrentTool == TOOL_FINGER)
					mStrokeInfoFinger.setBeautifyLineFillStyleIndex(fillStyle);				
			}
		});
	}
	...
	private void updateTouchUI(float x, float y, float pressure, int action, String tool){
		...
		if(mMappedPoint != null){
			if(mMappedPoint.x !=x || mMappedPoint.y!=y){
				mX.setText("X : " + String.format("%.2f", x) + "(" + String.format("%.2f", mMappedPoint.x) + ")");
				mY.setText("Y : " + String.format("%.2f", y) + "(" + String.format("%.2f", mMappedPoint.y) + ")");
			}
			else{
